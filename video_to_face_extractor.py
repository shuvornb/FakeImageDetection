"""
This source code was taken from this following github repository:
...
def saveFaces(generator, save_path, videono, batch_size=50):
    ...
    for epoch in range(n // batch_size + 1):
        ...
        i = 0
        for img in face_batch:
            image_r = Image.fromarray(img[:, :, 0]).convert('L')
            image_b = Image.fromarray(img[:, :, 1]).convert('L')
            image_g = Image.fromarray(img[:, :, 2]).convert('L')
            merged = Image.merge('RGB', (image_r, image_b, image_g))
            imgname = save_path + 'image_' + str(videono) + '_' + str(
                i) + '.png'
            i += 1
            merged.save(imgname)
...
@click.option("--image_shape", help="Shape of the cropped image")
def run(input_path, output_path, image_shape):
    dirname = input_path
    save_path = output_path
    image_size = int(image_shape)
    ...
    for vid in filenames:
        ...
        # print('Extracting Images from:  ', vid)
        gen = FaceBatchGenerator(face_finder, int(image_size))
        saveFaces(gen, save_path, videono)
